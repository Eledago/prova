...
/* 		sensor_msgs::JointState js_cartesian;
		js_cartesian.header = joint_state.header;
	    js_cartesian.header.stamp = ros::Time::now();
		js_cartesian.name.resize(6);
		js_cartesian.position.resize(6);
		js_cartesian.name[0] = "x";
		js_cartesian.position[0] = state->position[1];
		//std::cout << "js_cartesian pos: %f " << js_cartesian.position[0]  << std::endl;
		js_cartesian.name[1] = "y";
		js_cartesian.position[1] = state->position[2];
		js_cartesian.name[2] = "z";
		js_cartesian.position[2] = state->position[3];
		js_cartesian.name[3] = "vx";
		js_cartesian.position[3] = state->velocity[1];
		js_cartesian.name[4] = "vy";
		js_cartesian.position[4] = state->velocity[2];
		js_cartesian.name[5] = "vz";
		js_cartesian.position[5] = state->velocity[3];
		cart_pub.publish(js_cartesian); */
	}
};
...
/* 	float p[3] = { omni_state->position[0], omni_state->position[1],
			omni_state->position[2] };
	for (int i = 0; i < 3; i++)
		omni_state->pos[i] = p[i]; */

/* 	ROS_INFO("POS_x %.2f", omni_state->position[0]);
	ROS_INFO("POS_y %.2f", omni_state->position[1]);
	ROS_INFO("POS_z %.2f", omni_state->position[2]);
	ROS_INFO("POS_x %.2f", omni_state->joints[0]);
	ROS_INFO("POS_y %.2f", omni_state->joints[1]);
	ROS_INFO("POS_z %.2f", omni_state->joints[2]);
	ROS_INFO("JO_0 %.2f", omni_state->rot[0]);
	ROS_INFO("JO_1 %.2f", omni_state->rot[1]);
	ROS_INFO("JO_2 %.2f", omni_state->rot[2]); */
...
int main(int argc, char** argv) {
	...
	HDErrorInfo error;
	HHD hHD;
	//(device_name.c_str());//use ros param and set in launch file
